...
class ClassificationDataset(NLPDataset):
    ...
    def text_transform(self, text):
        ...
        return [self.vocab[t] for t in tokens[:self.max_length]]

    def label_binarizer(self, labels):
        label2idx = self.label2idx
        res = np.zeros(len(label2idx))
    ...
    def to_tfrecord(self, filename, dataset):
        ...
        def func(text, label):
            return tf.reshape(serialize_example((text, label),
                                                ('tensor', 'tensor')),
                              ())
    ...
    def parse_tfrecord(self, dataset):
        ...
        def func(record):
            ...
            return (
                tf.io.parse_tensor(parsed_record['feature0'], tf.float32),
                tf.io.parse_tensor(parsed_record['feature1'], tf.int32)
            )
