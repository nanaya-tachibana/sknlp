...
# https://github.com/tensorflow/models/blob/9b23daf9daaa1b44aee32e9f2e3b7ee5c9462b11/official/nlp/modeling/layers/transformer_encoder_block.py
# 复制一份方便修改
...
from sknlp.typing import WeightConstraint, WeightInitializer, WeightRegularizer
...
class TransformerEncoderBlock(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        inner_dim: int,
        inner_activation: Callable | str,
        output_range: int | None = None,
        kernel_initializer: WeightInitializer | None = "glorot_uniform",
        bias_initializer: WeightInitializer | None = "zeros",
        kernel_regularizer: WeightRegularizer | None = None,
        bias_regularizer: WeightRegularizer | None = None,
        activity_regularizer: WeightRegularizer | None = None,
        kernel_constraint: WeightConstraint | None = None,
        bias_constraint: WeightConstraint | None = None,
        use_bias: bool = True,
        ...
        attention_initializer: WeightInitializer | None = None,
        attention_axes: Sequence[int] | None = None,
        ...
        **kwargs
        ...
        self._enable_recompute_grad = enable_recompute_grad
    ...
    def get_config(self) -> dict[str, Any]:
        config = {
            ...
            "attention_axes": self._attention_axes,
            "enable_recompute_grad": self._enable_recompute_grad,
        }
    ...
    def call(self, inputs: list[tf.Tensor]) -> tf.Tensor:
        ...
        if isinstance(inputs, (list, tuple)):
            ...
            raise ValueError(
                "The type of input argument is expected to be sequence, got: %s"
                % type(inputs)
            )

        if key_value is None:
            key_value = input_tensor
        elif self._norm_first and key_value is not None:
            key_value = self._attention_layer_norm(key_value)

        def recompute_call(
            input_tensor: tf.Tensor, key_value: tf.Tensor, attention_mask: tf.Tensor
        ) -> tf.Tensor:
        ...
        if self._enable_recompute_grad:
            recompute_call = tf.recompute_grad(recompute_call)

        return recompute_call(input_tensor, key_value, attention_mask)